...
const ActionAuth = () => {
  ...
  return (
    <div className="flex justify-between items-center min-w-[110px]">
        <button className="p-2 px-3 rounded-full trans-second hover:bg-primary-15-color">
            <i className="bi bi-bell"></i>
        </button>
        <HeaderCart/>
        <div className="container-account ml-3">
            {(localStorage.getItem('user')) ? <div className="">
                <button className="my-2" onClick={handleLogin}>
                    <img className="w-10 h-10 object-cover bg-primary-15-color rounded-full hover:bg-primary-50-color" src="https://images.unsplash.com/photo-1644982647708-0b2cc3d910b7?ixlib=rb-1.2.1&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774&q=80" alt="" />
                </button>
                {openLogin && (

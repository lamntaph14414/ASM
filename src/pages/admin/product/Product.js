...
import {formatPrice} from '../../../utils/formatNumer'
const Product = () => {
    ...
    const colums = [
        {field: 'index', headerName: '#', with: 50},
        {
            field: 'name',
        ...
        },
        {
            ...
            sortable: false,
            renderCell(params){
                return <div className="w-20 h-20 "><img className="object-cover w-full h-full rounded p-2" sr={`${params.row.img}`}/></div>
            },
        },
        {
            field: 'price',
            ...
            editable:true,
            renderCell(params){
                return <div className='truncate'>{(params.row.salePrice) ? formatPrice(params.row.salePrice): formatPrice(params.row.regularPrice)}</div>
            }
        },
        {
            field: 'cate',
    ...
    <>
        <div className="container px-6 grid grid-cols-2">
            ...
            <div style={{ height: 460, width: '100%', background: "white" }}>
                <DataGrid rows= {newData} colums= {colums} pageSize={6} checkboxSelection disableSelectionOnClick />
            </div>
        </div>
    </>
  )
